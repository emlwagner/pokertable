...
public class Hand {
	...
	public void HandleJokerWilds() {
		...
		int CardNo = 0;
		for (Hand hand : hands) {
			hands = ExplodeHands(hands, CardNo);
			CardNo++;
		}
		for (Hand hand : hands) {
			hand.EvalHand();
		...
		this.HandStrength = hands.get(0).getHandStrength();
		this.HiHand = hands.get(0).getHighPairStrength();
		this.LoHand = hands.get(0).getLowPairStrength();
		this.Kicker = hands.get(0).getKicker();
	}
	...
	private static ArrayList<Hand> ExplodeHands(ArrayList<Hand> hands,
		...
		for (Hand h : hands) {
			...
			if (c.get(CardNo).getRank().getRank() == eRank.JOKER.getRank()) {
				for (Card card : jDeck.getCards()) {
					ArrayList<Card> SubCards = new ArrayList<Card>();
					SubCards.add(card);
					for (int i = 0; i < c.size(); i++) {
						if (i != CardNo) {
							SubCards.add(c.get(i));
						}
					}
					h.CardsInHand = SubCards;
					SubHands.add(h);
				}
			} else {
				SubHands.add(h);
	...
	public void EvalHand() {
		...
		// Five of a Kind
				...
				&& CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank() == CardsInHand
						.get(eCardNo.FourthCard.getCardNo()).getRank() && CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank()
